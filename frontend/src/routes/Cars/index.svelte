      ...
      if(action === "new"){
        ...
        try {
            ...
            const response = await fetch(import.meta.env.VITE_API_URL + "/api/cars/" + car_id, {
                ...
                body: JSON.stringify({meta, user: driver === "" ? null : driver, status }),
            });
            const data = await response.json();
            console.log("data: ", data)
            if(data.error){
...
<div class="container mx-auto py-6 px-3">
  <div class="flex justify-between items-center mb-6">
    <div>
      ...
      <p class="text-gray-500">{cars.filter(x => x.status === "free").length} available cars</p>
    </div>
    <button
          ...
          <tr class="{index % 2 === 0 ? 'bg-white' : 'bg-gray-100'} border-b border-l">
            ...
            <td class="py-3 px-4 border-r">
              ...
              {:else if car.status === "on_break"}
                <span class="text-yellow-900 bg-yellow-200 px-4 py-1 rounded-full inline-block">On break</span>
              {:else}
                <span class="text-red-900 bg-red-200 px-4 py-1 rounded-full inline-block">Busy</span>
              {/if}
            </td>
            <td class="py-3 px-4 border-r">{moment(car.created_at).format("DD/MM/YYYY HH:MM")}</td>
...
{#if show_form}
  ...
  <div transition:fade={{duration: 300}} class="fixed inset-0 z-40 flex items-center justify-center bg-white bg-opacity-80 backdrop-blur-sm transition-opacity duration-500">
    ...
    <div class="relative max-w-screen-lg w-full max-h-[80vh] overflow-y-auto bg-white p-8 rounded-xl shadow-xl border-2 z-50 transform transition-all duration-500">
      ...
      <h2 class="text-3xl font-bold text-center mb-6">{action === "new" ? "Add a new Car" : "Edit a Car"}</h2>
